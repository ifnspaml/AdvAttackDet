...
class MAEGradLoss(nn.Module):
    def __init__(self):
        super(MAEGradLoss, self).__init__()
    ...
    def forward(self, disp, img):
        ...
        grad_disp_x = torch.abs(grad_disp_x-grad_img_x)
        grad_disp_y = torch.abs(grad_disp_y-grad_img_y)

        return grad_disp_x.mean() + grad_disp_y.mean()


class SSIMGradLoss(nn.Module):
    """adapted from https://github.com/nianticlabs/monodepth2
    Layer to compute the SSIM loss between a pair of images
    """
    def __init__(self):
        super(SSIMGradLoss, self).__init__()
        self.mu_1_x_pool = nn.AvgPool2d(3, 1)
        self.mu_2_x_pool = nn.AvgPool2d(3, 1)
        self.mu_1_y_pool = nn.AvgPool2d(3, 1)
        self.mu_2_y_pool = nn.AvgPool2d(3, 1)

        self.sig_1_x_pool = nn.AvgPool2d(3, 1)
        self.sig_2_x_pool = nn.AvgPool2d(3, 1)
        self.sig_1_y_pool = nn.AvgPool2d(3, 1)
        self.sig_2_y_pool = nn.AvgPool2d(3, 1)
        self.sig_1_xy_pool = nn.AvgPool2d(3, 1)
        self.sig_2_xy_pool = nn.AvgPool2d(3, 1)

        self.refl_1 = nn.ReflectionPad2d(1)
        self.refl_2 = nn.ReflectionPad2d(1)
    ...
    def forward(self, disp, img):
        ...
        x = self.refl_1(grad_disp_x)
        y = self.refl_1(grad_img_x)
        mu_x = self.mu_1_x_pool(x)
        mu_y = self.mu_1_y_pool(y)
        sigma_x = self.sig_1_x_pool(x ** 2) - mu_x ** 2
        sigma_y = self.sig_1_y_pool(y ** 2) - mu_y ** 2
        sigma_xy = self.sig_1_xy_pool(x * y) - mu_x * mu_y
        SSIM_n = (2 * mu_x * mu_y + self.C1) * (2 * sigma_xy + self.C2)
        SSIM_d = (mu_x ** 2 + mu_y ** 2 + self.C1) * (sigma_x + sigma_y + self.C2)
        output_1 = torch.clamp((1 - SSIM_n / SSIM_d) / 2, 0, 1)

        x = self.refl_2(grad_disp_y)
        y = self.refl_2(grad_img_y)
        mu_x = self.mu_2_x_pool(x)
        mu_y = self.mu_2_y_pool(y)
        sigma_x = self.sig_2_x_pool(x ** 2) - mu_x ** 2
        sigma_y = self.sig_2_y_pool(y ** 2) - mu_y ** 2
        sigma_xy = self.sig_2_xy_pool(x * y) - mu_x * mu_y
        SSIM_n = (2 * mu_x * mu_y + self.C1) * (2 * sigma_xy + self.C2)
        SSIM_d = (mu_x ** 2 + mu_y ** 2 + self.C1) * (sigma_x + sigma_y + self.C2)
        output_2 = torch.clamp((1 - SSIM_n / SSIM_d) / 2, 0, 1)

        return output_1.mean() + output_2.mean()
